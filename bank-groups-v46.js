...
async function load(){
 ...
 integrateBell();addGroupsTab();enhancePiggies();
}
...
function integrateBell(){
 ...
 const b=document.querySelector('#notifications');if(!b)return;
 b.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();showNotifications()};
 const unread=notifications.filter(n=>!n.read_at).length;
 b.querySelector('.badge')?.remove();
 if(unread)b.insertAdjacentHTML('beforeend',`<span class="badge">${unread}</span>`);
}
...
function showNotifications(){
 const l=openLayer(`<div class="modal-head"><div><h2>Notificaciones</h2><p class="muted">Todas integradas en la campana original.</p></div><button class="close-btn" data-v46-close>×</button></div><div class="v46-notifications">${notifications.length?notifications.map(n=>`<article class="${n.read_at?'':'unread'}"><div><strong>${esc(n.title||'Notificación')}</strong><p>${esc(n.body||'')}</p><small>${new Date(n.created_at).toLocaleString('es-ES')}</small></div><div>${actionFor(n)}${!n.read_at?`<button class="btn" data-read="${n.id}">Leída</button>`:''}</div></article>`).join(''):'<div class="empty">No tienes notificaciones.</div>'}</div>`);
 l.querySelectorAll('[data-read]').forEach(b=>b.onclick=()=>rpcReload('mark_notification_read',{p_notification_id:b.dataset.read},b));
 l.querySelectorAll('[data-pay]').forEach(b=>b.onclick=()=>rpcReload('a2c_pay_group_split',{p_split_id:b.dataset.pay},b,'Pago registrado.'));
 l.querySelectorAll('[data-accept]').forEach(b=>b.onclick=()=>rpcReload('a2c_respond_group_invitation',{p_group_id:b.dataset.accept,p_accept:true},b,'Te has unido al grupo.'));
 l.querySelectorAll('[data-reject]').forEach(b=>b.onclick=()=>rpcReload('a2c_respond_group_invitation',{p_group_id:b.dataset.reject,p_accept:false},b,'Invitación rechazada.'));
}
async function rpcReload(fn,args,b,msg){busy(b,true);const {error}=await sb.rpc(fn,args);busy(b,false);if(error)return toast(error.message,true);toast(msg||'Actualizado.');await load();showNotifications()}
