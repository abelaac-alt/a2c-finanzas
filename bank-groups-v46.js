 ...
 notificationsChannel=sb.channel(`a2c-notifications-${me.id}`)
   .on('postgres_changes',{
     ...
     try{
       ...
       const panel=document.querySelector('#a2c-v46-layer .v46-notifications');
...
function integrateBell(){
 ...
 if(bell.dataset.v463Unified!=='1'){
   const clean=bell.cloneNode(true);
   clean.dataset.v463Unified='1';
   bell.replaceWith(clean);
   bell=clean;
   bell.addEventListener('click',event=>{
     ...
     document.querySelectorAll('#a2c-v46-layer').forEach((node,index)=>{if(index>0)node.remove()});
     showNotifications();
   });
 }
 updateBellBadge();
...
async function showNotifications(){
 document.querySelector('#modal')?.remove();
 document.querySelectorAll('#a2c-v46-layer').forEach(n=>n.remove());
 const l=openLayer(`<div class="modal-head"><div><h2>Notificaciones</h2><p class="muted" id="v46-notification-status">Actualizando…</p></div><button class="close-btn" data-v46-close>×</button></div><div class="v46-notifications"><div class="empty">Cargando notificaciones…</div></div>`);
 const box=l.querySelector('.v46-notifications');
 try{
   await refreshNotifications();
   const status=l.querySelector('#v46-notification-status');
   if(status)status.textContent=`${notifications.filter(n=>!n.read_at).length} sin leer`;
   renderNotificationRows(box);
 }catch(error){
   box.innerHTML='<div class="empty">No se pudieron cargar las notificaciones.</div>';
   toast(error.message||'No se pudieron cargar las notificaciones.',true);
 }
}
async function rpcReload(fn,args,b,msg){
 ...
 await refreshNotifications();
 const box=document.querySelector('#a2c-v46-layer .v46-notifications');
...
document.addEventListener('submit',event=>{
 ...
 if(piggyExpenseLocks.has(form))return;
 piggyExpenseLocks.add(form);

 const submitter=event.submitter||form.querySelector('button[type="submit"],button:not([type])');
 if(submitter)submitter.disabled=true;

 const mainHandler=form.onsubmit;
 if(typeof mainHandler!=='function'){
   piggyExpenseLocks.delete(form);
   if(submitter)submitter.disabled=false;
   return toast('No se pudo guardar el gasto. Recarga la aplicación.',true);
 }

 Promise.resolve(mainHandler.call(form,{
   preventDefault(){},
   stopPropagation(){},
   ...
   console.error('Error al guardar el gasto de hucha:',error);
   toast(error?.message||'No se pudo guardar el gasto.',true);
 }).finally(()=>{
   setTimeout(()=>{
     piggyExpenseLocks.delete(form);
