...
## Corrección de relaciones de Supabase

La consulta de `ledger_transactions` usa explícitamente las claves foráneas:

- `ledger_transactions_owner_id_fkey` para el perfil propietario;
- `ledger_transactions_folder_id_fkey` para la carpeta.

Esto evita el error de PostgREST «more than one relationship was found».
