(function(){
  ...
  A.auth={
    render(){
      A.root.innerHTML=`<section class="auth-page"><form class="auth-card" id="login-form">
        <div class="brand"><img src="./logo-a2c.png" alt="A2C"><div><h1>A2C Finanzas</h1><p>Control financiero claro y compartido</p></div></div>
      ...
      const form=A.root.querySelector('#login-form');
      const remembered=localStorage.getItem('a2c_identifier')||'';
    ...
    async login(event){
      ...
      const identifier=String(form.elements.identifier.value||'').trim().toLowerCase();
      const password=String(form.elements.password.value||'');A.setBusy(button,true,'Entrando…');
      try{
        let session=null;
        if(identifier.includes('@')&&!identifier.startsWith('@')){
          const result=await A.sb.auth.signInWithPassword({email:identifier,password});
          if(result.error)throw result.error;session=result.data.session;
        }else{
          const result=await A.sb.functions.invoke('secure-login',{body:{identifier,password}});
          if(result.error||!result.data?.session)throw new Error('Usuario o contraseña incorrectos.');
          const set=await A.sb.auth.setSession({access_token:result.data.session.access_token,refresh_token:result.data.session.refresh_token});
          if(set.error)throw set.error;session=set.data.session;
        }
    ...
    async start(){
      const session=await A.requireAuth();
      if(!session){A.state.user=null;A.auth.render();return;}
      A.state.user=session.user;
      await A.store.load();
      const hash=location.hash.replace('#','');A.state.page=['home','tools','messages','activity','profile'].includes(hash)?hash:'home';
      A.ui.render();
    },
    async signOut(){
      await A.sb.auth.signOut();window.A2CNative?.clearAuthSession?.();A.state.user=null;A.state.profile=null;A.auth.render();
    }
  };
  A.sb.auth.onAuthStateChange(async(event,session)=>{
    ...
    if(event==='SIGNED_IN'&&session){A.state.user=session.user;await A.store.load();A.ui.render();}
