(function(){
  ...
  A.store={
      ...
      loadingPromise=(async()=>{
        ...
        results.forEach((result,index)=>{
          ...
          if(result.status==='rejected'||result.value?.error){
            ...
            if(key==='transfers')A.state.transfers=[];
            else A.state[key]=[];
            return;
          }
        ...
        if(Date.now()-lastTouch>600000){
          sessionStorage.setItem('a2c_last_seen_touch',String(Date.now()));
          (async()=>{try{await A.sb.rpc('a2c_v7_touch_last_seen',{p_device:A.platform});}catch(error){console.warn(error);}})();
        }
    ...
    async statistics(from,to,{force=false}={}){
      ...
      const data=await A.rpc('a2c_v8_statistics',{p_from:from,p_to:to});
