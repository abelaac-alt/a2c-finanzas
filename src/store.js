(function(){
  ...
  A.store={
    async load({force=false}={}){
      ...
      loadingPromise=(async()=>{
        ...
        const tasks={
          ...
          resources:A.sb.from('resources').select('*').is('archived_at',null).order('created_at',{ascending:false}),
          transactions:A.sb.from('finance_transactions').select('*').order('occurred_on',{ascending:false}).order('created_at',{ascending:false}),
          transfers:A.sb.from('a2c_resource_transfers_v82').select('*').order('occurred_on',{ascending:false}).order('created_at',{ascending:false}),
        ...
        try{A.state.resourceMembers=await A.rpc('a2c_v82_list_resource_members');}catch(error){console.warn('Miembros de huchas no disponibles',error);A.state.resourceMembers=[];}
    ...
    async statistics(from,to,{force=false}={}){
      ...
      const data=await A.rpc('a2c_v82_statistics',{p_from:from,p_to:to});cache.set(key,{at:Date.now(),data});return data;
    },
    ...
    membersForResource(id){return A.state.resourceMembers.filter(row=>String(row.resource_id)===String(id));},
