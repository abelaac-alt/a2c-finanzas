...
async function openExpenseSplitNotification(notification){
  ...
  if(!detail)return toast('El gasto compartido ya no está disponible.',true);

  const modalCard=document.querySelector('#modal .modal-card');
  if(!modalCard)return;

  modalCard.innerHTML=`<div class="modal-head">
    <div>
      <span class="eyebrow">Gasto compartido</span>
      <h2>${esc(detail.concept||'Gasto compartido')}</h2>
      <p class="muted">${esc(detail.owner_name||'Un usuario')}${detail.resource_name?` · ${esc(detail.resource_name)}`:''}</p>
    </div>
    <button class="close-btn" data-close>×</button>
  </div>
  <div class="v52-expense-detail">
    <div><span>Importe pendiente</span><strong>${money(detail.amount_cents)}</strong></div>
    <div><span>Fecha</span><strong>${esc(detail.occurred_on||'')}</strong></div>
  </div>
  <div class="actions">
    <button type="button" class="btn" data-close>Cerrar</button>
    ${detail.paid_at?'<span class="status-chip">Pagado</span>':`<button type="button" class="btn primary" data-v52-pay="${detail.split_id}">Pagar</button>`}
  </div>`;
  ...
  modalCard.querySelector('[data-v52-pay]')?.addEventListener('click',async event=>{
    const button=event.currentTarget;
    busy(button,true);
    const {error}=await sb.rpc('a2c_pay_expense_split_v52',{
      p_split_id:button.dataset.v52Pay
    });
    busy(button,false);
    if(error)return toast(error.message,true);
...
function renderNotificationsInsideOriginal(modalCard,rows){
  ...
  modalCard.querySelectorAll('[data-v48-open]').forEach(button=>{
    button.onclick=()=>{
      ...
      if(notification.type==='expense_split'||notification.type==='expense_reminder'){
        openExpenseSplitNotification(notification);
        return;
      }
  ...
  modalCard.querySelectorAll('[data-v48-pay]').forEach(button=>{
    button.onclick=async event=>{
      event.stopPropagation();
      busy(button,true);
      const {error}=await sb.rpc('a2c_pay_expense_split_v52',{p_split_id:button.dataset.v48Pay});
      busy(button,false);
      if(error)return toast(error.message,true);
...
async function showPendingSharedExpenses(){
  document.querySelectorAll('.v48-pending-expenses,.v50-pending-expenses,.v51-pending-expenses').forEach(node=>node.remove());
}
