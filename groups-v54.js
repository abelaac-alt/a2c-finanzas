...
async function deleteGroup(group){
  ...
  const {data,error}=await sb.rpc('a2c_delete_group_with_refund_v60',{p_group_id:group.id});
  if(error)return toast(error.message,true);
  toast(Number(data||0)>0?`Grupo eliminado. Se han devuelto ${money(Number(data||0))} al propietario.`:'Grupo eliminado.');
  await load();
  renderGroups();
}
...
function openGroup(id){
  ...
  <div class="v58-group-actions">
    <button class="btn primary" data-income>Ingresar dinero</button>
    <button class="btn" data-expense>Registrar gasto</button>
    ${g.allow_withdrawals?'<button class="btn v61-withdraw-main" data-withdraw-main>Retirar a cuenta principal</button><button class="btn v58-transfer" data-transfer>Transferir a otra cuenta</button>':'<button class="btn v58-transfer" disabled title="Las retiradas están desactivadas">Retiradas desactivadas</button>'}
  </div>
  ...
  m.querySelector('[data-withdraw-main]')?.addEventListener('click',()=>openGroupTransfer(g,true));
  m.querySelector('[data-transfer]')?.addEventListener('click',()=>openGroupTransfer(g,false));
...
function openGroupTransfer(g,directMain=false){
  ...
  const m=modal(`<form><div class="modal-head"><div><h2>${directMain?'Retirar a cuenta principal':'Transferir dinero'}</h2><p class="muted">Disponible en el grupo: ${money(available)}</p></div><button type="button" class="close-btn" data-v47-close>×</button></div>
    ${directMain?'<input type="hidden" name="destination" value="main:"><div class="v61-transfer-route"><span>Grupo</span><b>→</b><span>Cuenta principal</span></div>':`<div class="field"><label>Cuenta de destino</label><select name="destination">${destinations}</select></div>`}
