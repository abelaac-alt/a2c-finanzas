...
function balance(id){return txs.filter(t=>sameId(t.group_id,id)).reduce((sum,t)=>sum+(t.kind==='income'?Number(t.amount_cents||0):-Number(t.amount_cents||0)),0)}
...
function openGroup(id){
  ...
  <div class="v58-group-actions"><button class="btn primary" data-income>Ingresar dinero</button><button class="btn" data-expense>Registrar gasto</button><button class="btn v58-transfer" data-transfer>Retirar / transferir</button></div>
  ...
  m.querySelector('[data-transfer]').onclick=()=>openGroupTransfer(g);
