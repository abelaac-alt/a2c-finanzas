...
if(!cfg.SUPABASE_URL||!cfg.SUPABASE_ANON_KEY){
  ...
  async function loadResources(force=false){
    if(resourcesCache.size&&!force)return resourcesCache;
    if(resourcesLoadPromise&&!force)return resourcesLoadPromise;

    resourcesLoadPromise=(async()=>{
      const {data,error}=await sb.from('resources').select('id,name,type');
      if(error)throw error;
      resourcesCache=new Map((data||[]).map(resource=>[String(resource.id),resource]));
  ...
  document.addEventListener('submit',async event=>{
    ...
    try{
      let resource=resourcesCache.get(String(resourceId));
      if(!resource){
        await loadResources(true);
        resource=resourcesCache.get(String(resourceId));
      }

      const selectedOption=
        form.elements.resource_id?.selectedOptions?.[0]
        ||form.elements.saving_goal_id?.selectedOptions?.[0];
      const fallbackName=String(selectedOption?.textContent||'destino').replace(/\s*·\s*(Hucha|Carpeta|Objetivo)\s*$/i,'').trim();

      const {error}=await sb.rpc('a2c_move_money_between_main_and_resource',{
        p_resource_id:resourceId,
        p_direction:'in',
        p_amount_cents:amount,
        p_concept:String(form.elements.concept?.value||`Aportación a ${resource?.name||fallbackName}`).trim(),
