...
function openTransaction(tx={}){
  ...
  modal(`<form id="tx-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nuevo'} movimiento</h2><p class="muted">Registra el concepto, el importe y los detalles necesarios.</p></div><button type="button" class="close-btn" data-close>×</button></div>
    ...
    <div class="field receipt-picker"><label>Justificante escaneado</label><div class="receipt-picker-actions"><label class="btn receipt-source-btn" for="receipt-camera">📷 Escanear con cámara</label><label class="btn receipt-source-btn" for="receipt-gallery">▣ Elegir foto o PDF</label>${editing&&tx.receipt_path?'<button type="button" class="btn receipt-source-btn receipt-open-existing" id="open-existing-receipt">Ver / descargar PDF</button>':''}</div><input id="receipt-camera" name="receipt_camera" class="receipt-file-input" type="file" accept="image/*" capture="environment"><input id="receipt-gallery" name="receipt_gallery" class="receipt-file-input" type="file" accept="image/*,application/pdf"><small class="muted receipt-selection" id="receipt-selection">${editing&&tx.receipt_path?'Hay un justificante guardado. Puedes conservarlo o sustituirlo.':'La cámara del dispositivo se abrirá directamente. La foto se mejorará y se convertirá automáticamente a PDF.'}</small></div>
...
async function getPermissionSnapshot(){
  const camera=await queryPermissionSafe('camera');
...
async function requestCameraPermission(){
  if(!navigator.mediaDevices?.getUserMedia){
    throw new Error('Este navegador no permite solicitar la cámara desde la web.');
  }

  // Cerramos el modal antes de pedir el permiso para que el aviso del navegador
  // nunca quede visualmente oculto detrás de la aplicación.
  closeModal();
  ...
  try{
    ...
    toast('Permiso de cámara concedido.');
    return true;
  }catch(error){
    if(error?.name==='NotAllowedError'){
      toast('La cámara está bloqueada. Debes permitirla desde los ajustes del navegador.',true);
    }else{
      toast(error?.message||'No se pudo comprobar la cámara.',true);
    }
    return false;
  }finally{
    stream?.getTracks?.().forEach(track=>track.stop());
    setTimeout(openAppSettings,300);
  }
}
  ...
  <section class="permission-center">
    ...
    <article class="permission-card">
      ...
      <div class="permission-copy">
        ...
        <small>Necesaria para fotografiar y escanear justificantes.</small>
      </div>
      <span class="permission-status ${permissionStatusClass(snapshot.camera)}">${permissionStatusLabel(snapshot.camera)}</span>
      <button type="button" class="btn primary" id="request-camera-permission">Solicitar cámara</button>
    </article>
