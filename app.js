...
async function scanReceiptToPdf(file){
  if(!(file instanceof File)||!file.size)throw new Error('Selecciona una foto válida.');
  if(file.type==='application/pdf')return file;
  if(!String(file.type||'').startsWith('image/'))throw new Error('El justificante debe ser una imagen o un PDF.');

  let bitmap=null;
  let objectUrl='';
  try{
    if(typeof createImageBitmap==='function'){
      try{bitmap=await createImageBitmap(file);}catch(error){console.warn('createImageBitmap falló; se usa compatibilidad Safari.',error);}
    }
    if(!bitmap){
      ...
      bitmap=await new Promise((resolve,reject)=>{
        ...
        image.onerror=()=>reject(new Error('No se pudo leer la fotografía tomada.'));
        image.src=objectUrl;
      });
    }
    ...
    if(!ctx)throw new Error('El navegador no pudo preparar el escáner.');
    ctx.fillStyle='#fff';
    ctx.fillRect(0,0,width,height);
    ctx.drawImage(bitmap,0,0,width,height);

    // Mejora tipo escáner: escala de grises suave, contraste y fondo más blanco.
    ...
    for(let index=0;index<data.length;index+=4){
      ...
      if(adjusted>205)adjusted=Math.min(255,adjusted+22);
      if(adjusted<55)adjusted=Math.max(0,adjusted-8);
      data[index]=data[index+1]=data[index+2]=Math.max(0,Math.min(255,adjusted));
      data[index+3]=255;
    }
    ctx.putImageData(image,0,0);

    const jpgBlob=await new Promise((resolve,reject)=>
      canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('No se pudo escanear la fotografía.')),'image/jpeg',0.9)
    );
    const jpgBytes=await jpgBlob.arrayBuffer();

    const pdf=await PDFDocument.create();
    pdf.setTitle('Justificante escaneado');
    pdf.setCreator('A2C Finanzas');
    pdf.setProducer('A2C Finanzas');
    const embedded=await pdf.embedJpg(jpgBytes);

    // Página A4. Se adapta la imagen manteniendo su proporción.
    ...
    const base=(file.name||'justificante').replace(/\.[^.]+$/,'').replace(/[^\w.-]+/g,'-');
    return new File([bytes],`${base}-escaneado.pdf`,{
...
async function prepareReceiptFile(file){
  ...
  if(file.type==='application/pdf')return file;
  return scanReceiptToPdf(file);
}

function receiptDownloadName(path){
  const clean=String(path||'').split('/').pop()||'justificante.pdf';
  return clean.toLowerCase().endsWith('.pdf')?clean:`${clean.replace(/\.[^.]+$/,'')}.pdf`;
}
  ...
  app.innerHTML=`<div class="app-shell">
    <header class="topbar">
      ...
      <div class="top-actions">
        ...
        <button class="icon-btn" id="app-settings" aria-label="Ajustes y permisos">${uiIcons.settings}</button>
        ${isAdmin()?`<button class="icon-btn admin-shortcut" id="admin" aria-label="Administración">${uiIcons.user}</button>`:''}
...
function toolsTabs(){const tabs=[['piggy','Huchas'],['folder','Carpetas'],['goal','Objetivos']];return `<nav class="section-tabs" aria-label="Herramientas">${tabs.map(([key,label])=>`<button class="${state.toolsSection===key?'active':''}" data-tools-section="${key}">${label}</button>`).join('')}</nav>`;}
function renderTools(){return `<section class="hub-page"><div class="dashboard-head"><div><span class="eyebrow">Organización</span><h1>Herramientas</h1><p class="muted">Gestiona huchas, carpetas, objetivos y movimientos automáticos.</p></div><div class="head-actions"><button type="button" class="btn primary" data-manage-recurring>Movimientos programados</button></div></div>${toolsTabs()}${renderResources(state.toolsSection)}</section>`;}
...
function txRow(tx){
  ...
  return `<article class="transaction-row clickable" data-edit-tx="${tx.id}" tabindex="0">
    ...
    <div class="transaction-tail"><b class="${tx.kind}">${tx.kind==='income'?'+':'−'}${money(tx.amount_cents)}</b><div class="transaction-mini-actions">${splitCount?`<span class="tx-split-badge">${splitCount} personas</span>`:''}${tx.receipt_path?`<button type="button" class="receipt-thumb-btn" data-receipt-path="${esc(tx.receipt_path)}" aria-label="Ver o descargar justificante" title="Ver o descargar justificante">${String(tx.receipt_path).toLowerCase().endsWith('.pdf')?'PDF':'🖼️'}</button>`:''}</div></div>
  </article>`;
}
...
function openTransaction(tx={}){
  ...
  modal(`<form id="tx-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nuevo'} movimiento</h2><p class="muted">Registra el concepto, el importe y los detalles necesarios.</p></div><button type="button" class="close-btn" data-close>×</button></div>
    ...
    <div class="field receipt-picker"><label>Justificante escaneado</label><div class="receipt-picker-actions"><button type="button" class="btn receipt-source-btn" id="open-native-receipt-camera">📷 Escanear con cámara</button><label class="btn receipt-source-btn" for="receipt-gallery">▣ Elegir foto o PDF</label>${editing&&tx.receipt_path?'<button type="button" class="btn receipt-source-btn receipt-open-existing" id="open-existing-receipt">Ver / descargar PDF</button>':''}</div><input id="receipt-camera" name="receipt_camera" class="receipt-file-input" type="file" accept="image/*" capture="environment"><input id="receipt-gallery" name="receipt_gallery" class="receipt-file-input" type="file" accept="image/*,application/pdf"><small class="muted receipt-selection" id="receipt-selection">${editing&&tx.receipt_path?'Hay un justificante guardado. Puedes conservarlo o sustituirlo.':'Se abrirá la cámara nativa del dispositivo. No necesitas conceder un permiso web adicional dentro de la aplicación.'}</small></div>
  ...
  const cameraInput=document.querySelector('#receipt-camera'),galleryInput=document.querySelector('#receipt-gallery'),receiptSelection=document.querySelector('#receipt-selection');
  let pendingReceiptFile=null;
  const setReceiptSelection=file=>{
    pendingReceiptFile=file||null;
    if(receiptSelection)receiptSelection.textContent=file
      ?(file.type==='application/pdf'?`PDF seleccionado: ${file.name}`:`Foto seleccionada: ${file.name||'captura'}. Se convertirá a PDF al guardar.`)
      :(editing&&tx.receipt_path?'Se conservará el justificante actual.':'No se ha seleccionado ningún justificante.');
  };
  const preserveReceiptScroll=()=>{
    const modalCard=document.querySelector('.modal-card,.v47-modal-card');
  ...
  const restoreReceiptScroll=stateSnapshot=>{
    requestAnimationFrame(()=>{
      if(stateSnapshot?.modalCard)stateSnapshot.modalCard.scrollTop=stateSnapshot.modalTop;
      if(stateSnapshot?.view)stateSnapshot.view.scrollTop=stateSnapshot.viewTop;
      setTimeout(()=>{
        if(stateSnapshot?.modalCard)stateSnapshot.modalCard.scrollTop=stateSnapshot.modalTop;
        if(stateSnapshot?.view)stateSnapshot.view.scrollTop=stateSnapshot.viewTop;
      },180);
  ...
  let receiptScrollSnapshot=null;

  document.querySelector('#open-native-receipt-camera')?.addEventListener('click',event=>{
    event.preventDefault();
    event.stopPropagation();
    receiptScrollSnapshot=preserveReceiptScroll();
    document.activeElement?.blur?.();
    // Se usa el selector de cámara nativo. Esto evita que un aviso de getUserMedia
    // quede oculto detrás de la aplicación.
    setTimeout(()=>cameraInput?.click(),40);
  });

  cameraInput?.addEventListener('change',()=>{
    const file=cameraInput.files?.[0]||null;
    if(galleryInput)galleryInput.value='';
    setReceiptSelection(file);
    restoreReceiptScroll(receiptScrollSnapshot);
  });
  cameraInput?.addEventListener('cancel',()=>restoreReceiptScroll(receiptScrollSnapshot));
  ...
  galleryInput?.addEventListener('change',()=>{
    const file=galleryInput.files?.[0]||null;
    if(cameraInput)cameraInput.value='';
    setReceiptSelection(file);
    restoreReceiptScroll(receiptScrollSnapshot);
  });
  galleryInput?.addEventListener('cancel',()=>restoreReceiptScroll(receiptScrollSnapshot));
  document.querySelector('#open-existing-receipt')?.addEventListener('click',()=>openReceipt(tx.receipt_path));
...
async function openReceipt(path){
  ...
  try{
    ...
    if(error)throw error;
    if(!data)throw new Error('No se pudo descargar el justificante.');

    const objectUrl=URL.createObjectURL(data);
    const isPdf=data.type==='application/pdf'||String(path).toLowerCase().endsWith('.pdf');
    const filename=isPdf?receiptDownloadName(path):(String(path).split('/').pop()||'justificante.jpg');

    modal(`<div class="modal-head"><div><h2>Justificante</h2><p class="muted">${isPdf?'Documento escaneado en PDF':'Imagen adjunta'}</p></div><button type="button" class="close-btn" data-close>×</button></div>
      <div class="receipt-document-actions">
        <a class="btn primary" id="download-receipt-file" href="${esc(objectUrl)}" download="${esc(filename)}">Descargar archivo</a>
      </div>
      <div class="receipt-preview ${isPdf?'receipt-preview-pdf':''}">
        ${isPdf
          ?`<iframe src="${esc(objectUrl)}#toolbar=1&navpanes=0" title="Justificante PDF"></iframe>`
          :`<img src="${esc(objectUrl)}" alt="Justificante de la transacción">`}
      </div>`,true);

    const modalElement=document.querySelector('.modal');
    modalElement?.addEventListener('click',event=>{
      if(event.target===modalElement||event.target.closest('[data-close]')){
        setTimeout(()=>URL.revokeObjectURL(objectUrl),300);
      }
    },{once:true});
  }catch(error){
    toast(error.message||'No se pudo abrir el justificante.',true);
  }
}
...
function permissionStatusLabel(status){
  ...
  if(status==='prompt')return 'Pendiente';
  if(status==='native')return 'Cámara nativa';
  if(status==='unsupported')return 'No compatible';
  return 'Sin comprobar';
}

function permissionStatusClass(status){
  if(status==='granted')return 'is-granted';
  if(status==='denied')return 'is-denied';
  if(status==='prompt')return 'is-prompt';
  if(status==='native')return 'is-native';
  return 'is-unknown';
}

async function queryPermissionSafe(name){
  try{
...
async function getPermissionSnapshot(){
  // El escáner usa la cámara nativa mediante un selector de archivo.
  // Algunos navegadores móviles no exponen un estado fiable del permiso "camera".
  const camera='native';
  const notifications=typeof Notification==='undefined'
    ?'unsupported'
    :(Notification.permission==='default'?'prompt':Notification.permission);
  const geolocation=await queryPermissionSafe('geolocation');
  return {camera,notifications,geolocation};
}
...
function permissionHelpText(){
  ...
  if(isiOS){
    return standalone
      ?'En iPhone, si la cámara continúa bloqueada, abre primero la web en Safari, toca “aA” → Ajustes del sitio web → Cámara → Permitir. Después vuelve a abrir la aplicación instalada.'
      :'En Safari, toca “aA” en la barra de direcciones → Ajustes del sitio web → Cámara → Permitir.';
  }
  if(isAndroid){
    return 'En Android, abre la información del sitio desde el candado de la barra de direcciones → Permisos → Cámara → Permitir.';
  }
  return 'Abre el candado o icono de información junto a la dirección web y permite la cámara para este sitio.';
}

async function requestCameraPermission(){
  // Para escanear justificantes se usa un input nativo con capture="environment".
  // No se solicita getUserMedia, porque en Brave/PWA puede quedar bloqueado aunque
  // el selector nativo de cámara sí funcione.
  ...
  input.addEventListener('change',()=>{
    ...
    if(file){
      toast('La cámara nativa funciona correctamente.');
    }else{
      toast('No se seleccionó ninguna fotografía.',true);
    }
    finish();
  },{once:true});

  input.addEventListener('cancel',()=>{
    toast('Prueba de cámara cancelada.');
    finish();
  },{once:true});
...
async function requestNotificationPermission(){
  if(typeof Notification==='undefined'){
    toast('Las notificaciones no son compatibles con este navegador.',true);
    return false;
  }
  try{
    const result=await Notification.requestPermission();
    toast(result==='granted'?'Permiso de notificaciones concedido.':'Las notificaciones no se han permitido.',result!=='granted');
    await openAppSettings();
    return result==='granted';
  }catch(error){
    toast(error.message||'No se pudieron solicitar las notificaciones.',true);
    return false;
  }
}
...
async function requestLocationPermission(){
  if(!navigator.geolocation){
    toast('La ubicación no es compatible con este navegador.',true);
    return false;
  }
  return new Promise(resolve=>{
    navigator.geolocation.getCurrentPosition(
      async()=>{
        toast('Permiso de ubicación concedido.');
        await openAppSettings();
        resolve(true);
      },
      async error=>{
        toast(error.message||'No se permitió la ubicación.',true);
        await openAppSettings();
        resolve(false);
      },
...
async function openAppSettings(){
  ...
  modal(`<div class="modal-head">
    <div><h2>Ajustes y permisos</h2><p class="muted">Cada usuario debe aceptar personalmente los permisos del navegador.</p></div>
    <button type="button" class="close-btn" data-close>×</button>
  </div>

  <section class="permission-center">
    <div class="permission-intro">
      <strong>Centro de permisos</strong>
      <p>La aplicación no puede activar permisos automáticamente. El navegador o el sistema siempre debe mostrar una confirmación al usuario.</p>
    </div>
    ...
    <article class="permission-card">
      ...
      <div class="permission-copy">
        ...
        <small>El escáner abre la cámara nativa. No depende del permiso web de getUserMedia.</small>
      </div>
      <span class="permission-status ${permissionStatusClass(snapshot.camera)}">${permissionStatusLabel(snapshot.camera)}</span>
      <button type="button" class="btn primary" id="request-camera-permission">Probar cámara nativa</button>
    </article>
    ...
    <article class="permission-card">
      ...
      <div class="permission-copy">
        ...
        <small>Permite recibir avisos y recordatorios compatibles.</small>
      </div>
      <span class="permission-status ${permissionStatusClass(snapshot.notifications)}">${permissionStatusLabel(snapshot.notifications)}</span>
      <button type="button" class="btn" id="request-notification-permission">Solicitar notificaciones</button>
    </article>
    ...
    <article class="permission-card">
      ...
      <div class="permission-copy">
        ...
        <small>Permiso opcional. Actualmente no es necesario para las finanzas.</small>
      </div>
      <span class="permission-status ${permissionStatusClass(snapshot.geolocation)}">${permissionStatusLabel(snapshot.geolocation)}</span>
      <button type="button" class="btn" id="request-location-permission">Solicitar ubicación</button>
    </article>

    <div class="permission-help">
      <strong>Cuando un permiso aparece bloqueado</strong>
      <p>${esc(permissionHelpText())}</p>
    </div>

    <div class="permission-actions">
      <button type="button" class="btn" id="open-profile-from-settings">Mi perfil</button>
      ${isAdmin()?'<button type="button" class="btn" id="open-admin-from-settings">Administración</button>':''}
    </div>
  </section>`,true);

  document.querySelector('#request-camera-permission')?.addEventListener('click',requestCameraPermission);
  document.querySelector('#request-notification-permission')?.addEventListener('click',requestNotificationPermission);
  document.querySelector('#request-location-permission')?.addEventListener('click',requestLocationPermission);
  document.querySelector('#open-profile-from-settings')?.addEventListener('click',()=>{closeModal();openProfile();});
  document.querySelector('#open-admin-from-settings')?.addEventListener('click',()=>{closeModal();openAdmin();});
}
