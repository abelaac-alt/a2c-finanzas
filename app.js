...
const state = {
  ...
  filters: { query:'', kind:'', resourceId:'', categoryId:'', from:'', to:'' }
};
...
function txRow(tx){
  const category = tx.category ? ` · ${esc(tx.category.icon || '•')} ${esc(tx.category.name)}` : '';
  return `<article class="transaction-row clickable" data-edit-tx="${tx.id}">
    ...
    <div class="transaction-copy"><strong>${esc(tx.concept)}</strong><small>${esc(tx.occurred_on)}${category}${tx.resource?.name?` · ${esc(tx.resource.name)}`:' · Cuenta principal'}</small></div>
...
function monthlySeries(months=6){
  ...
  for(let i=months-1;i>=0;i--){
    ...
    const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
    const list=state.transactions.filter(t=>String(t.occurred_on).startsWith(key));
    const t=totals(list);
...
function renderHome(){
  ...
  const health=Math.max(0,Math.min(100,Math.round(50+rate/2-(month.expense>month.income?20:0))));
  return `<section class="dashboard">
    <div class="dashboard-head">
      ...
      <button class="btn primary" data-new-tx>＋ Nuevo movimiento</button>
    </div>
    ...
    <article class="hero pro-hero">
      ...
      <div class="health-score"><span>Salud financiera</span><b>${health}/100</b><i style="--score:${health}%"></i></div>
      <div class="quick-actions">
        <button class="quick-action income" data-quick="income">＋ Ingreso</button>
        <button class="quick-action expense" data-quick="expense">− Gasto</button>
        <button class="quick-action investment" data-quick="investment">↗ Inversión</button>
        <button class="quick-action saving" data-quick="saving">◎ Ahorro</button>
      </div>
    </article>
    ...
    <div class="dashboard-grid">
      ...
      <article class="card">
        ...
        <div class="list">${state.transactions.slice(0,6).map(txRow).join('')||'<div class="empty compact">No hay movimientos.</div>'}</div>
      </article>
    </div>
  </section>`;
}
...
function filtered(){
  ...
  return state.transactions.filter(t=>(!f.query||String(t.concept).toLowerCase().includes(f.query.toLowerCase()))&&(!f.kind||t.kind===f.kind)&&(!f.categoryId||t.category_id===f.categoryId)&&(!f.resourceId||t.resource_id===f.resourceId)&&(!f.from||t.occurred_on>=f.from)&&(!f.to||t.occurred_on<=f.to));
}
...
function renderStats(){
  const list=state.transactions.filter(t=>(!state.filters.from||t.occurred_on>=state.filters.from)&&(!state.filters.to||t.occurred_on<=state.filters.to));
  const t=totals(list),series=monthlySeries(12),categories=categorySpending(list);
  ...
  return `<section>
    <div class="dashboard-head">
      <div><span class="eyebrow">Inteligencia financiera</span><h1>Estadísticas</h1><p class="muted">Analiza tendencias, categorías y capacidad de ahorro.</p></div>
    ...
    <form class="stats-toolbar" id="stats-filter">
    ...
    <div class="kpi-grid">
      ...
      <article class="kpi-card"><span>Movimientos</span><strong>${list.length}</strong><small>En el periodo</small></article>
    </div>
    ...
    <div class="dashboard-grid">
      <article class="card">
        <div class="card-head"><div><h2>Gastos por categoría</h2><p class="muted">Dónde se concentra tu gasto</p></div></div>
        <div class="category-bars">${categories.length?categories.slice(0,8).map(c=>`<div class="category-bar"><div><span>${esc(c.icon)} ${esc(c.name)}</span><b>${money(c.value)}</b></div><div class="bar-track"><i style="width:${Math.round(c.value/maxCategory*100)}%"></i></div></div>`).join(''):'<div class="empty compact">No hay gastos en este periodo.</div>'}</div>
      </article>
      <article class="card">
        <div class="card-head"><div><h2>Resumen del periodo</h2><p class="muted">Composición del dinero</p></div></div>
        <div class="summary-ring" style="--rate:${Math.max(0,Math.min(100,rate))}%"><div><strong>${rate}%</strong><span>Tasa de ahorro</span></div></div>
        <div class="summary-list">
          <span><i class="dot income"></i>Ingresos <b>${money(t.income)}</b></span>
          <span><i class="dot expense"></i>Gastos <b>${money(t.expense)}</b></span>
          <span><i class="dot investment"></i>Inversión <b>${money(t.investment)}</b></span>
          <span><i class="dot saving"></i>Ahorro <b>${money(t.saving)}</b></span>
        </div>
      </article>
    </div>
  </section>`;
...
function bind(){
  ...
  if(af)af.oninput=()=>{const fd=new FormData(af);Object.assign(state.filters,{query:String(fd.get('query')||''),from:String(fd.get('from')||''),to:String(fd.get('to')||''),kind:String(fd.get('kind')||''),categoryId:String(fd.get('categoryId')||''),resourceId:String(fd.get('resourceId')||'')});document.querySelector('#activity-list').innerHTML=filtered().map(txRow).join('')||'<div class="empty">No hay movimientos.</div>';bind()};
...
function openTransaction(tx={}){
  ...
  modal(`<form id="tx-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nuevo'} movimiento</h2></div><button type="button" class="close-btn" data-close>×</button></div>
    <div class="field"><label>Tipo</label><select name="kind">${Object.entries(kindLabels).map(([k,label])=>`<option value="${k}" ${tx.kind===k?'selected':''}>${label}</option>`).join('')}</select></div>
    <div class="form-grid">
      <div class="field"><label>Categoría</label><select name="category_id"><option value="">Sin categoría</option>${state.categories.map(c=>`<option value="${c.id}" ${tx.category_id===c.id?'selected':''}>${esc(c.icon||'•')} ${esc(c.name)}</option>`).join('')}</select></div>
      <div class="field"><label>Comercio / entidad</label><input name="merchant" value="${esc(tx.merchant||'')}" placeholder="Ej. Supermercado"></div>
    </div>
  ...
  document.querySelector('#tx-form').onsubmit=async e=>{
    e.preventDefault();const b=e.submitter,fd=new FormData(e.currentTarget);busy(b,true);
    try{
      const payload={kind:fd.get('kind'),category_id:fd.get('category_id')||null,merchant:String(fd.get('merchant')||'').trim(),amount_cents:cents(fd.get('amount')),concept:String(fd.get('concept')).trim(),occurred_on:fd.get('date'),notes:String(fd.get('notes')||'')};
      let id=tx.id;
...
function exportCsv(){
  const rows=[['Fecha','Tipo','Concepto','Comercio','Categoría','Espacio','Importe']];
  for(const t of filtered()){
    rows.push([t.occurred_on,kindLabels[t.kind],t.concept,t.merchant||'',t.category?.name||'',t.resource?.name||'Cuenta principal',(Number(t.amount_cents)/100).toFixed(2)]);
  }
