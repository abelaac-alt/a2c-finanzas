...
async function openExpenseNotification(notification){
  ...
  modal(`<div class="modal-head"><div><span class="eyebrow">Gasto compartido</span><h2>${esc(detail.concept||'Gasto compartido')}</h2><p class="muted">${esc(detail.owner_name||'Un usuario')}${detail.resource_name?` · ${esc(detail.resource_name)}`:''}</p></div><button class="close-btn" data-close>×</button></div>
...
async function openNotifications(){
  ...
  modal(`<div class="modal-head"><div><h2>Notificaciones</h2><p class="muted">${state.notifications.length} pendiente${state.notifications.length===1?'':'s'}</p></div><button class="close-btn" data-close>×</button></div>
    <div class="list">${state.notifications.map(n=>`<article class="notification" data-notification-id="${n.id}">
      <button type="button" class="notification-main" data-open-note="${n.id}">
      ...
      <div class="notification-actions">
        ...
        ${(n.type==='expense_split'||n.type==='expense_reminder')?`<button class="btn primary" data-open-note="${n.id}">Ver y pagar</button>`:''}
