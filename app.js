...
function renderLogin(){
  app.innerHTML=`<section class="auth-shell"><form class="auth-card" id="login-form">
    ...
    <div class="field"><label>Email</label><input name="email" type="email" autocomplete="username" inputmode="email" maxlength="254" autocapitalize="none" spellcheck="false" required></div>
...
async function loadAll(){
  ...
  [state.resources,state.members,state.invitations,state.transactions,state.notifications,state.recurring,state.cryptoHoldings,state.cryptoLedger]=result.slice(0,8).map(r=>r.data||[]);
...
function txRow(tx){
  ...
  return `<article class="transaction-row clickable" data-edit-tx="${tx.id}" tabindex="0">
    ...
    <div class="transaction-tail"><b class="${tx.kind}">${tx.kind==='income'?'+':'−'}${money(tx.amount_cents)}</b><div class="transaction-mini-actions">${tx.receipt_path?`<button type="button" class="receipt-thumb-btn" data-receipt-path="${esc(tx.receipt_path)}" aria-label="Ver imagen adjunta" title="Ver imagen">🖼️</button>`:''}</div></div>
  </article>`;
}
...
function renderResources(type){
  ...
  return `<section><div class="section-head"><div><h2>${meta[0]}</h2><p class="muted">${meta[1]}</p></div><button class="btn primary" data-new-resource="${type}">Nuevo</button></div>
    <div class="grid two">${list.map(r=>{
      const balance=resourceBalance(r.id),resourceMembers=state.members.filter(m=>m.resource_id===r.id&&m.profile),members=resourceMembers.length,pct=r.target_cents?Math.min(100,Math.round(Math.max(0,balance)/r.target_cents*100)):0;
...
function openFloatingTransactionMenu(){
  modal(`<div class="modal-head"><div><h2>Nuevo movimiento</h2><p class="muted">Selecciona el tipo de operación.</p></div><button class="close-btn" data-close>×</button></div>
    <div class="movement-type-grid">
      ...
      <button class="movement-type investment" data-fab-kind="investment"><span>◆</span><strong>Inversión</strong></button><button class="movement-type crypto" id="pay-crypto-fast"><span>₿</span><strong>Pagar con cripto</strong></button>
    </div>`);
  document.querySelectorAll('[data-fab-kind]').forEach(b=>b.onclick=()=>openTransaction({kind:b.dataset.fabKind}));document.querySelector('#pay-crypto-fast')?.addEventListener('click',()=>openCryptoPayment(null));
}
...
function openTransaction(tx={}){
  ...
  modal(`<form id="tx-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nuevo'} movimiento</h2><p class="muted">Registra el concepto, el importe y los detalles necesarios.</p></div><button type="button" class="close-btn" data-close>×</button></div>
    ...
    <div class="actions">${editing?'<button type="button" class="btn danger" id="delete-tx">Borrar</button><button type="button" class="btn repeat-action-btn" id="repeat-tx-modal"><span>🔁</span> Repetir</button>':''}<button type="button" class="btn" data-close>Cancelar</button><button class="btn primary">Guardar</button></div></form>`,true);
  ...
  const update=()=>{const saving=kind.value==='saving',investment=kind.value==='investment',fuel=kind.value==='expense'&&isFuelConcept(concept.value);document.querySelector('#saving-goal-field').classList.toggle('hidden',!saving);document.querySelector('#investment-detail').classList.toggle('hidden',!investment);document.querySelector('#fuel-detail').classList.toggle('hidden',!fuel);if(investment&&form.elements.investment_company&&!form.elements.investment_company.value)form.elements.investment_company.value=concept.value;const selected=state.resources.find(r=>r.id===resource.value);document.querySelector('#piggy-transfer-note').textContent=selected?.type==='piggy'&&(kind.value==='income'||saving)?'Esta aportación se restará automáticamente de la cuenta principal.':selected?.type==='folder'?'Este movimiento afectará al saldo de la cuenta principal.':'';syncFuel();syncInvestment();};
...
function openResourceMenu(id){
  ...
  modal(`<div class="modal-head"><div><h2>${esc(r.name)}</h2><p class="muted">${r.is_shared?'Compartido':'Personal'}</p></div><button class="close-btn" data-close>×</button></div><div class="menu-stack">
    <button class="btn" id="resource-edit">Editar</button>${r.is_shared?'<button class="btn" id="resource-invite">Invitar usuario</button>':''}<button class="btn" id="resource-view">Ver movimientos</button><button class="btn" id="resource-add-crypto">Agregar criptos</button><button class="btn" id="resource-pay-crypto">Pagar con cripto</button>
  </div>`);
  ...
  document.querySelector('#resource-view').onclick=()=>{state.filters.resourceId=id;state.tab='activity';closeModal();renderShell()};document.querySelector('#resource-add-crypto').onclick=()=>openCryptoTransfer(id);document.querySelector('#resource-pay-crypto').onclick=()=>openCryptoPayment(id);
}
...
function openCryptoPayment(resourceId=null){
  const available=state.cryptoHoldings.filter(h=>Number(h.quantity)>0&&String(h.resource_id||'')===String(resourceId||''));
  if(!available.length)return toast(resourceId?'Este elemento no tiene criptomonedas disponibles.':'No tienes criptomonedas disponibles en la cuenta principal.',true);
  modal(`<form id="crypto-payment-form"><div class="modal-head"><div><h2>Pagar con cripto</h2><p class="muted">El gasto se registra en euros, pero no reduce tu saldo bancario.</p></div><button type="button" class="close-btn" data-close>×</button></div><div class="field"><label>Criptomoneda</label><select name="holding_id" required>${available.map(h=>`<option value="${h.id}">${esc(holdingLabel(h))} · ${Number(h.quantity).toLocaleString('es-ES',{maximumFractionDigits:8})}</option>`).join('')}</select></div><div class="form-grid"><div class="field"><label>Cantidad utilizada</label><input name="quantity" inputmode="decimal" required></div><div class="field"><label>Valor por unidad al pagar (€)</label><input name="unit_price" inputmode="decimal" required></div></div><div class="field"><label>Concepto del pago</label><input name="concept" required placeholder="Ej. Compra online"></div><div class="field"><label>Fecha</label><input name="date" type="date" value="${today()}" required></div><div class="field"><label>Notas</label><textarea name="notes"></textarea></div><div class="actions"><button type="button" class="btn" data-close>Cancelar</button><button class="btn primary">Registrar pago</button></div></form>`);
  document.querySelector('#crypto-payment-form').onsubmit=async e=>{e.preventDefault();const b=e.submitter,fd=new FormData(e.currentTarget),holding=available.find(h=>h.id===fd.get('holding_id')),qty=cryptoQty(fd.get('quantity')),unit=cents(fd.get('unit_price'));if(!holding||qty<=0||qty>Number(holding.quantity))return toast('La cantidad no es válida o supera el saldo disponible.',true);if(unit<=0)return toast('Indica un valor por unidad válido.',true);busy(b,true);const {error}=await sb.rpc('a2c_spend_crypto',{p_symbol:holding.symbol,p_quantity:qty,p_unit_price_cents:unit,p_resource_id:holding.resource_id||null,p_concept:String(fd.get('concept')).trim(),p_occurred_on:fd.get('date'),p_notes:String(fd.get('notes')||'')});busy(b,false);if(error)return toast(error.message,true);closeModal();await refresh();toast('Pago con cripto registrado');};
}
...
function openUserProfile(profileId){
  const profile=findProfile(profileId);if(!profile)return toast('No se pudo cargar el perfil.',true);
  modal(`<div class="modal-head"><div><h2>Perfil de usuario</h2><p class="muted">Miembro de un espacio compartido</p></div><button type="button" class="close-btn" data-close>×</button></div><div class="public-profile">${avatarMarkup(profile,'profile-avatar-large')}<h3>${esc(profile.display_name||'Usuario')}</h3><p>${esc(profile.email||'')}</p></div>`);
}
function openProfile(){
  ...
  form.onsubmit=async e=>{e.preventDefault();const b=e.submitter,fd=new FormData(form);busy(b,true);try{
    ...
    const {data,error}=await sb.functions.invoke('account-settings',{body:{display_name:String(fd.get('name')||''),avatar_path:avatarPath,password:p}});
