...
function txRow(tx){
  ...
  return `<article class="transaction-row clickable" data-edit-tx="${tx.id}" tabindex="0">
    ...
    <div class="transaction-copy"><strong>${esc(tx.concept)}</strong><small>${esc(tx.occurred_on)} · ${space} · ${payment}${tx.is_transfer?' · Traspaso':''}${fuel}${investment}</small></div>
    <div class="transaction-tail"><b class="${tx.kind}">${tx.kind==='income'?'+':'−'}${money(tx.amount_cents)}</b><div class="transaction-mini-actions">${tx.receipt_path?`<button type="button" class="receipt-thumb-btn" data-receipt-path="${esc(tx.receipt_path)}" aria-label="Ver imagen adjunta" title="Ver imagen">🖼️</button>`:''}<button type="button" class="repeat-tx-btn" data-repeat-tx="${tx.id}" aria-label="Repetir ${esc(kindLabels[tx.kind]||'movimiento')}" title="Repetir movimiento">🔁</button></div></div>
  </article>`;
}
...
function renderStats(){
  ...
  const expenses=conceptSpending(list,'expense'),portfolio=investmentPortfolio(list),savings=conceptSpending(list,'saving'),fuel=fuelStatistics(list);
...
function bind(){
  ...
  document.querySelectorAll('[data-repeat-tx]').forEach(button=>button.onclick=event=>{event.preventDefault();event.stopPropagation();const source=state.transactions.find(tx=>tx.id===button.dataset.repeatTx);if(!source)return;openTransaction({...source,id:null,occurred_on:today(),receipt_path:null,is_transfer:false,transfer_group_id:null,transfer_role:null});});
...
function exportCsv(){
  const rows=[['Fecha','Tipo','Concepto / Empresa','ISIN','Acciones','Precio acción','Forma de pago','Espacio','Litros','Precio litro','Km','Consumo L/100 km','Importe']];
