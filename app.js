...
async function boot(){
  ...
  sb.auth.onAuthStateChange((_event,session)=>{
    state.user=session?.user||null;
    setTimeout(()=>{
      if(state.user)enter().catch(fatal);
      else renderLogin();
    },0);
  });
}
...
async function enter(){
  ...
  // La ejecución de programaciones no puede bloquear la entrada.
  setTimeout(async()=>{
    try{
      const {error}=await sb.rpc('a2c_process_my_scheduled_movements_v62');
      if(error)throw error;
      await loadAll();
      renderShell();
    }catch(error){
      console.warn('No se pudieron procesar los movimientos programados.',error);
    }
  },250);
}
...
function withTimeout(promise,ms=12000){
  return Promise.race([
    ...
    new Promise(resolve=>setTimeout(()=>resolve({data:[],error:new Error('Tiempo de espera agotado')}),ms))
  ]);
}
...
async function loadAll(){
  const q=[
    ...
    sb.from('scheduled_movements_v62').select('*').order('next_run'),
  ...
  const result=await Promise.all(q.map(query=>withTimeout(query)));
  ...
  [state.resources,state.members,state.invitations,state.transactions,state.notifications,state.recurring,state.cryptoHoldings,state.cryptoLedger,state.friendships,state.socialProfiles,state.expenseSplits,state.stockSales]=result.slice(0,12).map(row=>row.data||[]);
  state.socialPosts=[];state.socialLikes=[];state.socialComments=[];state.follows=[];state.leaderboard=[];
  ...
  state.expenseSplits=state.expenseSplits.filter(row=>String(row.owner_id)===String(state.user.id)||String(row.debtor_user_id)===String(state.user.id));
...
function renderTools(){return `<section class="hub-page"><div class="dashboard-head"><div><span class="eyebrow">Organización</span><h1>Herramientas</h1><p class="muted">Gestiona huchas, carpetas y objetivos desde un único lugar.</p></div></div>${toolsTabs()}${renderResources(state.toolsSection)}</section>`;}
...
function bind(){
  ...
  document.querySelectorAll('[data-follow-reject]').forEach(b=>b.onclick=()=>respondFollow(b.dataset.followReject,'rejected'));renderShell()});
...
function schedulePlaceLabel(type,id){
  if(type==='main')return 'Cuenta principal';
  if(type==='resource')return state.resources.find(item=>String(item.id)===String(id))?.name||'Elemento';
  if(type==='group')return 'Grupo compartido';
  return 'Destino';
}
function scheduleFrequencyLabel(row){
  if(row.frequency==='daily')return 'Todos los días';
  if(row.frequency==='weekly')return `Cada ${['domingo','lunes','martes','miércoles','jueves','viernes','sábado'][Number(row.weekday||0)]}`;
  return `Día ${row.day_of_month} de cada mes`;
}
function openRecurring(){
  modal(`<div class="modal-head"><div><h2>Movimientos programados</h2><p class="muted">Transferencias automáticas entre la cuenta principal y tus espacios.</p></div><button type="button" class="close-btn" data-close>×</button></div>
  <button class="btn primary full" id="new-recurring">Nuevo movimiento programado</button>
  <div class="scheduled-list">${state.recurring.length?state.recurring.map(row=>`
    <article class="scheduled-card">
      <div class="scheduled-route"><span>${esc(schedulePlaceLabel(row.source_type,row.source_id))}</span><b>→</b><span>${esc(schedulePlaceLabel(row.destination_type,row.destination_id))}</span></div>
      <div class="scheduled-main"><div><strong>${esc(row.concept)}</strong><small>${scheduleFrequencyLabel(row)} · Próximo: ${esc(row.next_run)}</small></div><b>${money(row.amount_cents)}</b></div>
      <div class="scheduled-actions"><button class="btn" data-edit-schedule="${row.id}">Editar</button><button class="btn danger" data-delete-schedule="${row.id}">Borrar</button></div>
    </article>`).join(''):'<div class="empty compact">No hay movimientos programados.</div>'}</div>`,true);
  document.querySelector('#new-recurring').onclick=()=>openRecurringForm();
  document.querySelectorAll('[data-edit-schedule]').forEach(button=>button.onclick=()=>openRecurringForm(state.recurring.find(row=>row.id===button.dataset.editSchedule)));
  document.querySelectorAll('[data-delete-schedule]').forEach(button=>button.onclick=async()=>{
    if(!confirm('¿Borrar este movimiento programado?'))return;
    const {error}=await sb.from('scheduled_movements_v62').delete().eq('id',button.dataset.deleteSchedule);
    if(error)return toast(error.message,true);
    await refresh(false);openRecurring();toast('Movimiento programado eliminado');
  });
}
async function openRecurringForm(existing=null){
  const {data:destinations,error}=await sb.rpc('a2c_schedule_destinations_v62');
  if(error)return toast(error.message,true);
  const places=[{type:'main',id:'',label:'Cuenta principal'},...(destinations||[]).map(item=>({type:item.place_type,id:item.place_id,label:item.place_label}))];
  const encode=(type,id)=>`${type}:${id||''}`;
  const options=places.map(item=>`<option value="${encode(item.type,item.id)}">${esc(item.label)}</option>`).join('');
  modal(`<form id="scheduled-form"><div class="modal-head"><div><h2>${existing?'Editar':'Nuevo'} movimiento programado</h2><p class="muted">Uno de los dos extremos debe ser la cuenta principal.</p></div><button type="button" class="close-btn" data-close>×</button></div>
    <div class="scheduled-direction-grid"><div class="field"><label>Desde</label><select name="source">${options}</select></div><div class="scheduled-arrow">→</div><div class="field"><label>Hacia</label><select name="destination">${options}</select></div></div>
    ...
    <div class="field" id="scheduled-day-wrap"><label>Día del mes</label><input name="day_of_month" type="number" min="1" max="28" value="${existing?.day_of_month||1}"></div>
    <div class="field hidden" id="scheduled-weekday-wrap"><label>Día de la semana</label><select name="weekday"><option value="1">Lunes</option><option value="2">Martes</option><option value="3">Miércoles</option><option value="4">Jueves</option><option value="5">Viernes</option><option value="6">Sábado</option><option value="0">Domingo</option></select></div></div>
  ...
  form.elements.source.value=encode(existing?.source_type||'main',existing?.source_id||'');
  form.elements.destination.value=encode(existing?.destination_type||places.find(item=>item.type!=='main')?.type||'resource',existing?.destination_id||places.find(item=>item.type!=='main')?.id||'');
  form.elements.frequency.value=existing?.frequency||'monthly';
  form.elements.weekday.value=String(existing?.weekday??1);
  const sync=()=>{document.querySelector('#scheduled-day-wrap').classList.toggle('hidden',form.elements.frequency.value!=='monthly');document.querySelector('#scheduled-weekday-wrap').classList.toggle('hidden',form.elements.frequency.value!=='weekly');};
  form.elements.frequency.onchange=sync;sync();
  form.onsubmit=async event=>{
    event.preventDefault();const button=event.submitter,fd=new FormData(form);
    const [sourceType,sourceId]=String(fd.get('source')).split(':');
    const [destinationType,destinationId]=String(fd.get('destination')).split(':');
    if(sourceType===destinationType&&String(sourceId||'')===String(destinationId||''))return toast('El origen y el destino no pueden ser iguales.',true);
    if(sourceType!=='main'&&destinationType!=='main')return toast('Uno de los dos extremos debe ser la cuenta principal.',true);
    const payload={user_id:state.user.id,source_type:sourceType,source_id:sourceId||null,destination_type:destinationType,destination_id:destinationId||null,concept:String(fd.get('concept')||'').trim(),amount_cents:cents(fd.get('amount')),frequency:String(fd.get('frequency')),day_of_month:fd.get('frequency')==='monthly'?Number(fd.get('day_of_month')||1):null,weekday:fd.get('frequency')==='weekly'?Number(fd.get('weekday')||1):null,next_run:String(fd.get('next_run')),active:fd.get('active')==='on'};
    busy(button,true);
    const result=existing?await sb.from('scheduled_movements_v62').update(payload).eq('id',existing.id):await sb.from('scheduled_movements_v62').insert(payload);
    busy(button,false);
    if(result.error)return toast(result.error.message,true);
    closeModal();await refresh(false);openRecurring();toast(existing?'Programación actualizada':'Movimiento programado creado');
  };
}
