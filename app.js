...
async function loadAll(){
  const q=[
    ...
    sb.from('social_post_likes').select('*').order('created_at',{ascending:false}),
    sb.from('social_post_comments').select('*,author:profiles!social_post_comments_user_id_fkey(id,display_name,username,avatar_path)').order('created_at',{ascending:true}),
  ...
  [state.resources,state.members,state.invitations,state.transactions,state.notifications,state.recurring,state.cryptoHoldings,state.cryptoLedger,state.socialPosts,state.socialLikes,state.socialComments,state.friendships,state.follows,state.socialProfiles,state.expenseSplits,state.stockSales,state.leaderboard]=result.slice(0,17).map(r=>r.data||[]);
  state.profiles=isAdmin()?(result[17]?.data||[]):[];
...
function bind(){
  ...
  document.querySelectorAll('[data-open-own-post]').forEach(b=>b.onclick=()=>{const post=state.socialPosts.find(p=>p.id===b.dataset.openOwnPost);if(post){modal(`<div class="modal-head"><div><h2>@${esc(state.profile.username||'usuario')}</h2></div><button class="close-btn" data-close>×</button></div>${socialPostMarkup(post)}`,true);bindSocialPostActions(document.querySelector('#modal'));}});
...
function openStockSale(tx=null){const editing=Boolean(tx?.id),existing=editing?saleForTransaction(tx.id):null,portfolio=stockPortfolio();if(!editing&&!portfolio.length)return toast('No tienes acciones disponibles para vender.',true);const selected=existing?.isin||portfolio[0]?.isin||'';modal(`<form id="stock-sale-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nueva'} venta de acciones</h2><p class="muted">Los impuestos y comisiones son configurables según tu extracto real.</p></div><button type="button" class="close-btn" data-close>×</button></div><div class="field"><label>Empresa / ISIN</label><select name="isin" ${editing?'disabled':''}>${portfolio.map(r=>`<option value="${esc(r.isin)}" ${selected===r.isin?'selected':''}>${esc(r.company)} · ${esc(r.isin)} · ${r.shares.toLocaleString('es-ES',{maximumFractionDigits:8})} acc.</option>`).join('')}${editing&&!portfolio.some(r=>r.isin===selected)?`<option value="${esc(selected)}" selected>${esc(existing.company)} · ${esc(selected)}</option>`:''}</select></div><div class="form-grid"><div class="field"><label>Cantidad de acciones</label><input name="quantity" inputmode="decimal" required value="${existing?.quantity||''}"></div><div class="field"><label>Precio de venta (€)</label><input name="sell_price" inputmode="decimal" required value="${existing?.sell_unit_price_cents?existing.sell_unit_price_cents/100:''}"></div><div class="field"><label>Comisiones (€)</label><input name="commission" inputmode="decimal" value="${existing?.commission_cents?existing.commission_cents/100:'0'}"></div><div class="field"><label>Impuestos / retenciones (€)</label><input name="tax" inputmode="decimal" value="${existing?.tax_cents?existing.tax_cents/100:'0'}"></div></div><div class="field"><label>Fecha</label><input name="date" type="date" required value="${existing?.occurred_on||today()}"></div><div class="field"><label>Notas</label><textarea name="notes">${esc(existing?.notes||'')}</textarea></div><div class="stock-sale-preview" id="stock-sale-preview"></div><div class="actions">${editing?'<button type="button" class="btn danger" id="delete-stock-sale">Borrar</button><button type="button" class="btn" id="download-stock-sale">Descargar extracto</button><button type="button" class="btn" id="share-stock-sale">Compartir</button>':''}<button type="button" class="btn" data-close>Cancelar</button><button class="btn primary">Guardar venta</button></div></form>`,true);const form=document.querySelector('#stock-sale-form');const current=()=>{const isin=editing?existing.isin:form.elements.isin.value,row=portfolio.find(r=>r.isin===isin),quantity=positive(form.elements.quantity.value),sell=cents(form.elements.sell_price.value),commission=Math.max(0,cents(form.elements.commission.value)),tax=Math.max(0,cents(form.elements.tax.value)),buy=existing?.buy_unit_price_cents||Math.round(row?.average||0),calc=saleCalculations(quantity,buy,sell,commission,tax);return{row,isin,quantity,sell,commission,tax,buy,calc};};const draw=()=>{const x=current();document.querySelector('#stock-sale-preview').innerHTML=`<div><span>Precio medio de compra</span><b>${money(x.buy)}</b></div><div><span>Ingreso neto</span><b>${money(x.calc.net)}</b></div><div><span>Beneficio neto</span><b class="${x.calc.profit>=0?'income':'expense'}">${money(x.calc.profit)}</b></div>`;};form.oninput=draw;draw();form.onsubmit=async e=>{e.preventDefault();const b=e.submitter,x=current();if(!(x.quantity>0)||!(x.sell>0))return toast('Indica cantidad y precio de venta válidos.',true);if(!editing&&x.quantity>Number(x.row?.shares||0))return toast('No tienes suficientes acciones disponibles.',true);busy(b,true);try{const args={p_transaction_id:editing?tx.id:null,p_isin:x.isin,p_company:existing?.company||x.row.company,p_quantity:x.quantity,p_sell_unit_price_cents:x.sell,p_commission_cents:x.commission,p_tax_cents:x.tax,p_occurred_on:form.elements.date.value,p_notes:form.elements.notes.value};const {error}=await sb.rpc(editing?'a2c_update_stock_sale':'a2c_create_stock_sale',args);if(error)throw error;closeModal();await refresh();toast(editing?'Venta recalculada':'Venta registrada');}catch(err){toast(err.message,true)}finally{busy(b,false)}};document.querySelector('#download-stock-sale')?.addEventListener('click',()=>downloadStockSaleReceipt(existing));document.querySelector('#share-stock-sale')?.addEventListener('click',async()=>{const file=await drawStockSaleReceipt(existing);openShareTransaction({...tx,receipt_path:null,_shareFile:file});});document.querySelector('#delete-stock-sale')?.addEventListener('click',async()=>{if(!confirm('¿Borrar esta venta?'))return;const {error}=await sb.rpc('a2c_delete_stock_sale',{p_transaction_id:tx.id});if(error)return toast(error.message,true);closeModal();await refresh();toast('Venta eliminada')});}
...
function openUserProfile(profileId){
  ...
  document.querySelectorAll('[data-open-post]').forEach(b=>b.onclick=()=>{const post=state.socialPosts.find(p=>p.id===b.dataset.openPost);if(post){modal(`<div class="modal-head"><div><h2>@${esc(profile.username||'usuario')}</h2></div><button class="close-btn" data-close>×</button></div>${socialPostMarkup(post)}`,true);bindSocialPostActions(document.querySelector('#modal'));}});
}
