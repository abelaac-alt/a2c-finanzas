...
function financeDonut(values){
  ...
  return `<div class="finance-donut-layout">
    <div class="finance-donut-wrap" id="finance-donut-wrap">
      ...
      <div class="finance-donut-center" id="finance-donut-center"><small>${total>0?'Pulsa una porción':'Sin movimientos este mes'}</small></div>
    </div>
  </div>`;
}
...
function bind(){
  ...
  const clearDonutSelection=()=>{const center=document.querySelector('#finance-donut-center');document.querySelectorAll('.finance-donut-slice.active').forEach(x=>x.classList.remove('active'));if(center)center.innerHTML='<small>Pulsa una porción</small>';};
