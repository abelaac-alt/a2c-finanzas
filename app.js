...
function txRow(tx){
  ...
  return `<article class="transaction-row clickable" data-edit-tx="${tx.id}" tabindex="0">
    ...
    <div class="transaction-tail"><b class="${tx.kind}">${tx.kind==='income'?'+':'−'}${money(tx.amount_cents)}</b>${tx.receipt_path?`<button type="button" class="receipt-thumb-btn" data-receipt-path="${esc(tx.receipt_path)}" aria-label="Ver justificante">▣</button>`:''}</div>
  </article>`;
}
...
function openTransaction(tx={}){
  ...
  modal(`<form id="tx-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nuevo'} movimiento</h2><p class="muted">Registra el concepto, el importe y los detalles necesarios.</p></div><button type="button" class="close-btn" data-close>×</button></div>
    ...
    <div class="field receipt-picker"><label>Justificante</label><div class="receipt-picker-actions"><label class="btn receipt-source-btn" for="receipt-camera">📷 Hacer foto</label><label class="btn receipt-source-btn" for="receipt-gallery">▣ Elegir imagen</label></div><input id="receipt-camera" name="receipt_camera" class="receipt-file-input" type="file" accept="image/*" capture="environment"><input id="receipt-gallery" name="receipt_gallery" class="receipt-file-input" type="file" accept="image/*"><small class="muted receipt-selection" id="receipt-selection">No se ha seleccionado ninguna imagen.</small></div>
  ...
  const setReceiptSelection=(source,other)=>{const file=source?.files?.[0]||null;if(file&&other)other.value='';if(receiptSelection)receiptSelection.textContent=file?`Imagen seleccionada: ${file.name||'foto tomada'}`:'No se ha seleccionado ninguna imagen.';};
  cameraInput?.addEventListener('change',()=>setReceiptSelection(cameraInput,galleryInput));
  galleryInput?.addEventListener('change',()=>setReceiptSelection(galleryInput,cameraInput));
