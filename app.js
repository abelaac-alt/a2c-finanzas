...
function renderShell(){
  ...
  app.innerHTML=`<div class="app-shell">
    <header class="topbar">
      <button class="brand-compact" id="profile" aria-label="Abrir perfil"><span class="brand-mark">A2C</span></button>
...
function renderHome(){
  const month=totals(monthTransactions()),rate=savingsRate();
  const due=state.recurring.filter(r=>r.active&&r.next_date).slice(0,4);
  return `<section class="dashboard">
    <div class="dashboard-head"><div><span class="eyebrow">Resumen financiero</span><h1>Hola, ${esc(state.profile.display_name||'')}</h1><p class="muted">Tu dinero, sin ruido y de un vistazo.</p></div></div>
    <article class="hero pro-hero"><div><span>Patrimonio disponible</span><strong>${money(mainBalance())}</strong><small>Cuenta principal</small></div></article>
    <div class="kpi-grid">
      ...
      <article class="kpi-card"><span>Ahorro del mes</span><strong class="saving">${money(month.saving)}</strong><small>Tasa: ${rate}%</small></article>
      <article class="kpi-card"><span>Inversión del mes</span><strong class="investment">${money(month.investment)}</strong><small>Capital invertido</small></article>
    </div>
    <article class="card chart-card"><div class="card-head"><div><h2>Evolución mensual</h2><p class="muted">Ingresos y gastos de los últimos 6 meses</p></div><div class="chart-key"><span class="key-income">Ingresos</span><span class="key-expense">Gastos</span></div></div>${professionalLineChart(monthlySeries())}</article>
    <div class="dashboard-grid">
      <article class="card"><div class="card-head"><div><h2>Próximos movimientos</h2><p class="muted">Operaciones recurrentes</p></div><button class="text-btn" data-manage-recurring>Gestionar</button></div><div class="list">${due.length?due.map(r=>`<div class="mini-row"><div><strong>${esc(r.concept)}</strong><small>${esc(r.next_date)} · ${kindLabels[r.kind]}</small></div><b>${money(r.amount_cents)}</b></div>`).join(''):'<div class="empty compact">No hay movimientos recurrentes próximos.</div>'}</div></article>
      <article class="card"><div class="card-head"><div><h2>Últimos movimientos</h2><p class="muted">Actividad reciente</p></div><button class="text-btn" data-tab-shortcut="activity">Ver todos</button></div><div class="list">${state.transactions.filter(t=>!(t.is_transfer&&t.transfer_role==='destination')).slice(0,6).map(txRow).join('')||'<div class="empty compact">No hay movimientos.</div>'}</div></article>
    </div>
  </section>`;
}
