...
function txRow(tx){
  const category=tx.kind==='expense'&&tx.category?`${esc(tx.category.icon||'•')} ${esc(tx.category.name)}`:'Sin categoría';
  ...
  return `<article class="transaction-row clickable" data-edit-tx="${tx.id}" tabindex="0">
    ...
    <div class="transaction-copy"><strong>${esc(tx.concept)}</strong><small>${esc(tx.occurred_on)} · ${category} · ${space} · ${payment}${tx.is_transfer?' · Traspaso':''}</small></div>
