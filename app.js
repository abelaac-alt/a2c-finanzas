...
function financeDonut(values){
  ...
  const slices=items.map(item=>{
    ...
    const slice=`<circle class="finance-donut-slice" cx="80" cy="80" r="${radius}" fill="none" stroke="${item.color}" stroke-width="24" stroke-dasharray="${length} ${Math.max(0,circumference-length)}" stroke-dashoffset="${-offset}" data-donut-key="${item.key}" data-donut-label="${item.label}" data-donut-value="${item.value}" tabindex="0" role="button" aria-label="${item.label}: ${money(item.value)}"></circle>`;
    offset+=length;
    return slice;
  }).join('');
  return `<div class="finance-donut-layout">
    <div class="finance-donut-wrap">
      <svg class="finance-donut" viewBox="0 0 160 160" role="img" aria-label="Distribución financiera del mes">
        <circle class="finance-donut-track" cx="80" cy="80" r="${radius}" fill="none" stroke-width="24"></circle>
        <g transform="rotate(-90 80 80)">${slices}</g>
      </svg>
      <div class="finance-donut-center" id="finance-donut-center"><small>Total del mes</small><strong>${money(total)}</strong></div>
    </div>
    <div class="finance-donut-legend">${items.map(item=>`<button type="button" class="donut-legend-item" data-donut-key="${item.key}" data-donut-label="${item.label}" data-donut-value="${item.value}"><i style="background:${item.color}"></i><span>${item.label}</span><b>${money(item.value)}</b></button>`).join('')}</div>
  </div>`;
}
...
function renderHome(){
  ...
  return `<section class="dashboard home-overview">
    ...
    <article class="hero pro-hero home-balance"><div><span>Patrimonio disponible</span><strong>${money(mainBalance())}</strong><small>Saldo disponible en la cuenta principal</small></div></article>
...
function bind(){
  ...
  const showDonutValue=el=>{const center=document.querySelector('#finance-donut-center');if(!center)return;const label=el.dataset.donutLabel||'Total';const value=Number(el.dataset.donutValue)||0;center.innerHTML=`<small>${esc(label)}</small><strong>${money(value)}</strong>`;document.querySelectorAll('[data-donut-key]').forEach(x=>x.classList.toggle('active',x.dataset.donutKey===el.dataset.donutKey));};
  document.querySelectorAll('[data-donut-key]').forEach(el=>{el.onclick=()=>showDonutValue(el);el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();showDonutValue(el)}}});
