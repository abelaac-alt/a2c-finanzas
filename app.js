...
async function loadAll(){
  ...
  result.forEach((row,index)=>{
    if(!row?.error)return;
    failed.push({index,error:row.error});
    console.warn(`A2C: consulta opcional ${index} no disponible`,row.error);
    result[index]={data:[]};
  });
  if(failed.length){
    console.warn(`A2C inició con ${failed.length} módulo(s) de datos no disponibles.`);
  }
  ...
  // v55: segunda barrera de privacidad en el cliente.
  // Supabase/RLS es la protección principal; este filtro evita representar
  // accidentalmente filas ajenas si una política se configura mal en el futuro.
  const belongsToCurrentUser=row=>{
    const owner=row?.user_id??row?.owner_id??row?.profile_id;
    return owner==null||String(owner)===String(state.user.id);
  };
  ...
  state.recurring=state.recurring.filter(belongsToCurrentUser);
  state.cryptoHoldings=state.cryptoHoldings.filter(belongsToCurrentUser);
  state.cryptoLedger=state.cryptoLedger.filter(belongsToCurrentUser);
  state.stockSales=state.stockSales.filter(belongsToCurrentUser);
  ...
  const socialPaths=[...new Set(state.socialPosts.map(post=>post.image_path).filter(Boolean))];
  if(socialPaths.length){
    try{
      const {data,error}=await sb.storage.from('social').createSignedUrls(socialPaths,3600);
      if(error)throw error;
      const urls=new Map((data||[]).map(row=>[row.path,row.signedUrl||row.signed_url||'']));
      state.socialPosts.forEach(post=>{post.signed_url=urls.get(post.image_path)||'';});
    }catch(error){
      console.warn('No se pudieron firmar las imágenes sociales en lote.',error);
      await Promise.all(state.socialPosts.slice(0,30).map(async post=>{if(!post.image_path)return;const {data}=await sb.storage.from('social').createSignedUrl(post.image_path,3600);post.signed_url=data?.signedUrl||'';}));
    }
  }
}
...
function renderShell(){
  ...
  app.innerHTML=`<div class="app-shell">
    ...
    <nav class="bottom-nav" aria-label="Navegación principal">${nav('home','Inicio')}${nav('tools','Herramientas')}${nav('activity','Actividad')}${nav('social','Social')}${nav('stats','Estadísticas')}</nav>
  </div>`;
  bind();
}
...
function openResourceMenu(id){
  ...
  const memberCount=Array.isArray(state.members)
    ? state.members.filter(member=>String(member.resource_id)===String(r.id)).length
    : 0;

  closeModal();
  document.querySelector('#resource-menu-modal')?.remove();
  document.body.insertAdjacentHTML('beforeend',`
    <div id="resource-menu-modal" class="v47-modal">
      <section class="v47-modal-card resource-menu-modal-card">
        <div class="modal-head">
          <div>
            <h2>${esc(r.name)}</h2>
            <p class="muted">${typeLabel} ${r.is_shared?'compartida':'personal'}${r.is_shared?` · ${memberCount} integrantes`:''}</p>
          </div>
          <button type="button" class="close-btn" data-resource-menu-close>×</button>
        </div>
        <div class="menu-stack v54-group-menu-stack">
          ...
          ${r.is_shared&&owner?'<button type="button" class="btn" data-resource-action="invite">Integrantes</button>':''}
          ${owner?`<button type="button" class="btn danger" data-resource-action="delete">Eliminar ${typeLabel.toLowerCase()}</button>`:''}
        </div>
      </section>
    </div>`);

  const menu=document.querySelector('#resource-menu-modal');
  const closeResourceMenu=()=>menu?.remove();
  menu.onclick=event=>{
    if(event.target===menu||event.target.closest('[data-resource-menu-close]'))closeResourceMenu();
  };

  menu.querySelectorAll('[data-resource-action]').forEach(button=>{
