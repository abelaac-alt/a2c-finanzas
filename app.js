...
function openTransaction(tx={}){
  ...
  modal(`<form id="tx-form"><div class="modal-head"><div><h2>${editing?'Editar':'Nuevo'} movimiento</h2><p class="muted">Registra el concepto, el importe y los detalles necesarios.</p></div><button type="button" class="close-btn" data-close>×</button></div>
    ...
    <div class="field receipt-picker"><label>Justificante escaneado</label><div class="receipt-picker-actions"><button type="button" class="btn receipt-source-btn" id="open-integrated-camera">📷 Escanear con cámara</button><label class="btn receipt-source-btn" for="receipt-gallery">▣ Elegir foto o PDF</label>${editing&&tx.receipt_path?'<button type="button" class="btn receipt-source-btn receipt-open-existing" id="open-existing-receipt">Ver / descargar PDF</button>':''}</div><input id="receipt-gallery" name="receipt_gallery" class="receipt-file-input" type="file" accept="image/*,application/pdf"><small class="muted receipt-selection" id="receipt-selection">${editing&&tx.receipt_path?'Hay un justificante guardado. Puedes conservarlo o sustituirlo.':'La fotografía se mejorará y se convertirá automáticamente a PDF.'}</small></div>
