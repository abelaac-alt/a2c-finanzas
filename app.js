...
function renderStats(){
  const list=statsTransactions(),t=totals(list),series=monthlySeries(12),categories=categorySpending(list);
  ...
  return `<section>
    ...
    <div class="dashboard-grid">
      <article class="card">
        <div class="card-head"><div><h2>Gastos por categoría</h2><p class="muted">Combustible, ocio, comida y otros</p></div></div>
...
function openTransaction(tx={}){
  ...
  const updateVisibility=()=>{
    ...
    transferNote.textContent=isDeposit?'Este ingreso en la hucha se restará automáticamente de la cuenta principal.':'';
  };
