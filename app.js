...
function openProfile(){
  ...
  form.onsubmit=async e=>{e.preventDefault();const b=e.submitter,fd=new FormData(form);busy(b,true);try{
    ...
    const {error}=await sb.from('profiles').update({display_name:String(fd.get('name')).trim(),avatar_path:avatarPath}).eq('id',state.user.id);if(error)throw error;
    const p=String(fd.get('password')||'');if(p){const {error:pe}=await sb.auth.updateUser({password:p});if(pe)throw pe}closeModal();await enter();toast('Perfil actualizado');
