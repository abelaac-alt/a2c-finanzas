...
const state = {
  ...
  socialPosts: [], friendships: [], socialProfiles: [],
...
async function loadAll(){
  ...
  [state.resources,state.members,state.invitations,state.transactions,state.notifications,state.recurring,state.cryptoHoldings,state.cryptoLedger,state.socialPosts,state.friendships,state.socialProfiles]=result.slice(0,11).map(r=>r.data||[]);
  state.profiles=isAdmin()?(result[11]?.data||[]):[];
...
function canSeePost(post){return post.user_id===state.user.id||post.author?.is_public||friendshipWith(post.user_id)?.status==='accepted';}
function socialImageUrl(post){return post?.signed_url||'';}
function renderSocial(){
  const posts=state.socialPosts.filter(canSeePost);
  const pending=state.friendships.filter(f=>f.addressee_id===state.user.id&&f.status==='pending');
  return `<section class="social-page"><div class="section-head"><div><h2>Comunidad</h2><p class="muted">Comparte momentos financieros con privacidad.</p></div><button class="btn" data-find-people>Buscar personas</button></div>
  ${pending.length?`<article class="card social-requests"><h3>Solicitudes</h3>${pending.map(f=>{const p=state.socialProfiles.find(x=>x.id===f.requester_id);return `<div class="row">${avatarMarkup(p,'small')}<div><strong>${esc(p?.display_name||'Usuario')}</strong><small>@${esc(p?.username||'usuario')}</small></div><div><button class="btn success" data-friend-accept="${f.id}">Aceptar</button><button class="btn danger" data-friend-reject="${f.id}">Rechazar</button></div></div>`}).join('')}</article>`:''}
  <div class="social-feed">${posts.length?posts.map(post=>{const image=socialImageUrl(post);const tx=post.transaction;return `<article class="social-post"><div class="social-post-head"><button class="social-author" data-social-profile="${post.user_id}">${avatarMarkup(post.author,'small')}<span><strong>${esc(post.author?.display_name||'Usuario')}</strong><small>@${esc(post.author?.username||'usuario')} · ${new Date(post.created_at).toLocaleDateString('es-ES')}</small></span></button>${post.user_id===state.user.id?`<button class="icon-btn" data-delete-post="${post.id}" aria-label="Borrar publicación">×</button>`:''}</div><div class="social-photo-wrap"><img src="${esc(image)}" alt="Publicación de ${esc(post.author?.display_name||'usuario')}" loading="lazy"><div class="social-overlay"><strong>${esc(post.caption||tx?.concept||'Movimiento')}</strong>${tx?`<span>${esc(kindLabels[tx.kind]||'Movimiento')} · ${money(tx.amount_cents)} · ${esc(tx.occurred_on)}</span>`:''}</div></div></article>`}).join(''):'<div class="empty">Todavía no hay publicaciones visibles.</div>'}</div></section>`;
}
...
function openPeopleSearch(){modal(`<div class="modal-head"><div><h2>Buscar personas</h2><p class="muted">Busca por nombre o @usuario.</p></div><button class="close-btn" data-close>×</button></div><div class="field"><input id="people-query" placeholder="@usuario o nombre"></div><div id="people-results" class="list"></div>`,true);const input=document.querySelector('#people-query'),box=document.querySelector('#people-results');const draw=()=>{const q=input.value.trim().toLowerCase().replace(/^@/,'');const rows=state.socialProfiles.filter(p=>p.id!==state.user.id&&(!q||String(p.username||'').includes(q)||String(p.display_name||'').toLowerCase().includes(q))).slice(0,30);box.innerHTML=rows.map(p=>{const f=friendshipWith(p.id);return `<div class="row">${avatarMarkup(p,'small')}<div><strong>${esc(p.display_name||'Usuario')}</strong><small>@${esc(p.username||'sin_usuario')} · ${p.is_public?'Pública':'Privada'}</small></div>${f?`<span class="status-chip">${f.status==='accepted'?'Amigos':f.status==='pending'?'Pendiente':'Rechazada'}</span>`:`<button class="btn" data-add-friend="${p.id}">Añadir</button>`}</div>`}).join('')||'<div class="empty compact">Sin resultados.</div>';box.querySelectorAll('[data-add-friend]').forEach(b=>b.onclick=()=>sendFriendRequest(b.dataset.addFriend));};input.oninput=draw;draw();}
...
function openShareTransaction(tx){modal(`<form id="share-post-form"><div class="modal-head"><div><h2>Compartir movimiento</h2><p class="muted">La información aparecerá superpuesta sobre la foto.</p></div><button type="button" class="close-btn" data-close>×</button></div><div class="share-preview-info"><strong>${esc(tx.concept)}</strong><span>${kindLabels[tx.kind]} · ${money(tx.amount_cents)} · ${esc(tx.occurred_on)}</span></div><div class="field"><label>Foto</label><input name="image" type="file" accept="image/*" required></div><div class="field"><label>Texto opcional</label><textarea name="caption" maxlength="180" placeholder="Añade un comentario…"></textarea></div><div class="actions"><button type="button" class="btn" data-close>Cancelar</button><button class="btn primary">Publicar</button></div></form>`);document.querySelector('#share-post-form').onsubmit=async e=>{e.preventDefault();const b=e.submitter,fd=new FormData(e.currentTarget),original=fd.get('image');if(!(original instanceof File)||!original.size)return toast('Selecciona una imagen.',true);busy(b,true);try{const file=await compressReceipt(original);const path=`${state.user.id}/${crypto.randomUUID()}.jpg`;const {error:up}=await sb.storage.from('social').upload(path,file,{contentType:'image/jpeg'});if(up)throw up;const {error}=await sb.from('social_posts').insert({user_id:state.user.id,transaction_id:tx.id,image_path:path,caption:String(fd.get('caption')||'').trim()});if(error)throw error;closeModal();await refresh();state.tab='social';renderShell();toast('Publicado');}catch(err){toast(err.message,true)}finally{busy(b,false)}};}
