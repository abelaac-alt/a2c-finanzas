...
function renderNotificationsInsideOriginal(modalCard,rows){
  ...
  modalCard.querySelectorAll('[data-v48-pay]').forEach(button=>{
    button.onclick=async event=>{
      event.stopPropagation();
      busy(button,true);
      const {error}=await sb.rpc('a2c_pay_expense_split_v51',{p_split_id:button.dataset.v48Pay});
      busy(button,false);
      if(error)return toast(error.message,true);
...
async function showPendingSharedExpenses(){
  ...
  const {data,error}=await sb.rpc('a2c_my_pending_expense_splits_v51');
  if(error){
  ...
  section.querySelectorAll('[data-v50-pay]').forEach(button=>{
    button.onclick=async()=>{
      busy(button,true);
      const {error}=await sb.rpc('a2c_pay_expense_split_v51',{p_split_id:button.dataset.v50Pay});
      busy(button,false);
      if(error)return toast(error.message,true);
...
setInterval(()=>{
  if(document.visibilityState==='visible')showPendingSharedExpenses();
},15000);
