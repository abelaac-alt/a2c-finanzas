...
async function showPendingSharedExpenses(){
  const {data,error}=await sb.from('expense_splits')
    .select('id,transaction_id,owner_id,debtor_user_id,amount_cents,paid_at,person_name')
    .eq('debtor_user_id',me.id)
    .is('paid_at',null)
    .order('created_at',{ascending:false});
  if(error||!data?.length)return;

  const page=document.querySelector('.view');
  if(!page||page.querySelector('.v48-pending-expenses'))return;

  const section=document.createElement('section');
  section.className='card v48-pending-expenses';
  section.innerHTML=`<div class="section-head"><div><h2>Gastos compartidos pendientes</h2><p class="muted">Gastos que otros usuarios han compartido contigo.</p></div></div>
  <div class="list">${data.map(row=>`
    <article class="row">
      <div><strong>${money(row.amount_cents)}</strong><small>Pendiente de pago</small></div>
      <button class="btn primary" data-v48-pay-row="${row.id}">Pagar</button>
    </article>`).join('')}</div>`;
  page.prepend(section);

  section.querySelectorAll('[data-v48-pay-row]').forEach(button=>{
    button.onclick=async()=>{
      busy(button,true);
      const {error}=await sb.rpc('a2c_pay_expense_split_v48',{p_split_id:button.dataset.v48PayRow});
      busy(button,false);
      if(error)return toast(error.message,true);
